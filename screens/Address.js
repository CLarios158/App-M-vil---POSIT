...
export default ({ navigation }) => {
    ...
    const apellido1 = navigation.getParam('apellido1');
    const apellido2 = navigation.getParam('apellido2');
    const curp = navigation.getParam('curp');
    const sexo = navigation.getParam('sexo');
    const fecha_nacimiento = navigation.getParam('date');
    ...
    const onSubmit = () => {
        ...
        if(cve_estado == '' || cve_municipio == '' || cve_asenta == '' || calle == ''){
            ...
            if (type == 1) {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('INSERT INTO persona(id_sexo,nombre,apellido1,apellido2,curp,fecha_nacimiento,fecha_creacion) VALUES (?,?,?,?,?,?,?);', [sexo, nombre, apellido1, apellido2, curp, fecha_nacimiento, fecha_registro], (tx, results) => {
                        id_person = results.insertId;
                    });

                }, Error => { Alert.alert('Error', 'Ha ocurrido un error!'), console.log(Error) });

                db.transaction((tx) => {
                    tx.executeSql('INSERT INTO encuesta(id_lugar,id_persona,cve_asenta,cve_municipio,cve_estado,calle,num_ext,num_int,fecha_creacion,grado,grupo) VALUES (?,?,?,?,?,?,?,?,?,?,?);', [id_lugar, id_person, cve_asenta, cve_municipio, cve_estado, calle, num_ext, num_int, fecha_registro, grado, grupo], (tx, results) => {
                        var id_encuest = results.insertId;
                        navigation.navigate('Question', { id_encuesta: id_encuest });
                    });
                ...
                let id_luga, id_person;

                db.transaction((tx) => {

                    tx.executeSql('INSERT INTO lugar(cve_asenta,cve_municipio,cve_estado,nombre_lugar,calle,num_ext,num_int,fecha_creacion) VALUES (?,?,?,?,?,?,?,?);',
                        [cve_asentaL, cve_municipioL, cve_estadoL, nombreL, calleL, num_extL, num_intL, fecha_registro], (tx, results) => {
                            // Get rows with Web SQL Database spec compliance.
                            id_luga = results.insertId;
                        });
                ...
                db.transaction((tx) => {
                    tx.executeSql('INSERT INTO encuesta(id_lugar,id_persona,cve_asenta,cve_municipio,cve_estado,calle,num_ext,num_int,fecha_creacion, grupo) VALUES (?,?,?,?,?,?,?,?,?,?);', [id_luga, id_person, cve_asenta, cve_municipio, cve_estado, calle, num_ext, num_int, fecha_registro, ''], (tx, results) => {
                        // Get rows with Web SQL Database spec compliance.
                        var id_encuest = results.insertId;
                        navigation.navigate('Question', { id_encuesta: id_encuest });
                    });
    ...
    if (loading == true) {
        ...
        return (
            <View style={styles.container}>
                <Text style={{ marginLeft: 25, marginRight: 25, marginTop: 20, }}>Por favor ingresa los datos de tu domicilio para continuar con tu registro.</Text>
